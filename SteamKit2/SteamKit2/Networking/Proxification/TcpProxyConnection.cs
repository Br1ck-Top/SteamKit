...
{
    class TcpProxyConnection : IConnection
    {
        ...
        public TcpProxyConnection( ILogContext log, Proxy proxy)
        {
            this.log = log ?? throw new ArgumentNullException( nameof( log ) );
            netLock = new object();
            socket = new ProxySocket( AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp );

            // Check for fiddler debugging, it doesn't work with TCP connection
            if ( proxy.Ip != "127.0.0.1" )
            {
                if ( proxy.Ip.Contains( "webshare" ) )
                {
                    socket.ProxyEndPoint = new IPEndPoint( Dns.GetHostEntry( proxy.Ip ).AddressList[ 0 ], proxy.Port );
                }
                else
                {
                    socket.ProxyEndPoint = new IPEndPoint( IPAddress.Parse( proxy.Ip ), proxy.Port );
                }
        ...
        private void ConnectCompleted( bool success )
        {
            ...
            {
                log.LogDebug( nameof( TcpProxyConnection ), "Connection request to {0} was cancelled", CurrentEndPoint );
                if ( success ) Shutdown();
            ...
            {
                log.LogDebug( nameof( TcpProxyConnection ), "Failed connecting to {0}", CurrentEndPoint );
                Release( userRequestedDisconnect: false );
                return;
            }

            log.LogDebug( nameof( TcpProxyConnection ), "Connected to {0}", CurrentEndPoint );
            DebugLog.Assert( socket != null, nameof( TcpProxyConnection ), "Socket should be non-null after connecting." );
            ...
            {
                log.LogDebug( nameof( TcpProxyConnection ), "Exception while setting up connection to {0}: {1}", CurrentEndPoint, ex );
                Release( userRequestedDisconnect: false );
            }
        }
        ...
        {
            DebugLog.Assert( cancellationToken != null, nameof( TcpProxyConnection ), "null CancellationToken in TryConnect" );

            if ( cancellationToken.IsCancellationRequested )
            {
                log.LogDebug( nameof( TcpProxyConnection ), "Connection to {0} cancelled by user", CurrentEndPoint );
                Release( userRequestedDisconnect: true );
                return;
            }

            DebugLog.Assert( socket != null, nameof( TcpProxyConnection ), "socket should not be null when connecting (we hold the net lock)" );
            DebugLog.Assert( CurrentEndPoint != null, nameof( TcpProxyConnection ), "CurrentEndPoint should be non-null when connecting." );
            ...
            {
                log.LogDebug( nameof( TcpProxyConnection ), "Exception while connecting to {0}: {1}", CurrentEndPoint, ex );
        ...
        public void Connect( EndPoint endPoint, int timeout )
        {
            lock ( netLock )
            {
                DebugLog.Assert( cancellationToken == null, nameof( TcpProxyConnection ), "Connection cancellation token is not null" );
                ...
                log.LogDebug( nameof( TcpProxyConnection ), "Connecting to {0}...", CurrentEndPoint );
                TryConnect( timeout );
            }
        ...
        void NetLoop()
        {
            ...
            DebugLog.Assert( cancellationToken != null, nameof( TcpProxyConnection ), "null cancellationToken in NetLoop" );
            ...
            while ( !cancellationToken.IsCancellationRequested )
            {
                ...
                {
                    log.LogDebug( nameof( TcpProxyConnection ), "Socket exception while polling: {0}", ex );
                    break;
                }
                ...
                {
                    log.LogDebug( nameof( TcpProxyConnection ), "Socket exception occurred while reading packet: {0}", ex );
                    break;
                }
                ...
                {
                    log.LogDebug( nameof( TcpProxyConnection ), "Unexpected exception propogated back to NetLoop: {0}", ex );
                }
            }
        ...
        byte[] ReadPacket()
        {
            ...
            if ( packetMagic != TcpProxyConnection.MAGIC )
            {
        ...
        public void Send( Memory<byte> data )
        {
            lock ( netLock )
            {
                ...
                {
                    log.LogDebug( nameof( TcpProxyConnection ), "Attempting to send client data when not connected." );
                    return;
                }
                ...
                {
                    log.LogDebug( nameof( TcpProxyConnection ), "Socket exception while writing data: {0}", ex );
                }
            }
        }
        ...
        public IPAddress GetLocalIP()
        {
            lock ( netLock )
            {
                ...
                {
                    log.LogDebug( nameof( TcpProxyConnection ), "Socket exception trying to read bound IP: {0}", ex );
                    return IPAddress.None;
                }
            }
        }
